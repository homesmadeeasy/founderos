...
assert.match(sql, /session_rpe/i)
assert.match(sql, /energy_after/i)
assert.match(sql, /discomfort_reported/i)
assert.match(sql, /bodyweight_kg/i)
assert.match(sql, /paused_rest_remaining_ms/i)
assert.match(sql, /current_exercise_key/i)
assert.match(sql, /original_prescription/i)
assert.match(sql, /\bfailed\b/i)
assert.match(sql, /status in \('planned', 'in_progress', 'paused', 'completed', 'skipped', 'cancelled'\)/i)
